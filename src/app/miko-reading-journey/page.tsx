        ...
        <div className="min-h-screen bg-paper-deep py-12 px-4">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="relative">
                    {/* Vertical line */}
                    <div className="absolute left-6 sm:left-8 top-0 bottom-0 w-0.5 bg-evergreen/20" />
                    ...
                    <div className="space-y-10">
                        {mikoBooks.map((book, index) => (
                            <div key={book.id} className="relative flex gap-4 sm:gap-6">
                                ...
                                {/* Content card */}
                                <div className="flex-1 bg-white rounded-2xl border overflow-hidden shadow-card" style={{ borderColor: "var(--hairline)" }}>
                                    <div className="flex flex-col sm:flex-row">
                                        ...
                                        <div className="p-5 flex-1 space-y-2">
                                            <div className="flex items-start justify-between gap-2">
                                                ...
                                                {index === 0 && (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-marigold text-ink text-xs font-bold whitespace-nowrap">
                                                        Start here
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center text-sm font-bold text-evergreen hover:underline mt-1"
                                            >
                                                View Book →
                                            </Link>
                                        </div>
                ...
                {/* CTA */}
                <div className="mt-16 text-center bg-evergreen rounded-3xl p-8 text-white">
                    <h2 className="font-heading text-2xl sm:text-3xl font-bold text-marigold mb-3">
                    ...
                    <Link
                        href="/books"
                        className="inline-flex items-center gap-2 bg-marigold text-ink font-extrabold px-8 py-4 rounded-full shadow-card hover:bg-marigold-deep transition-colors text-lg"
                    >
                        Shop the Bundle
                    </Link>
                </div>
                ...
                {/* NitiTales Feature */}
                <div className="mt-12 bg-white rounded-3xl border overflow-hidden shadow-card" style={{ borderColor: "var(--hairline)" }}>
                    <div className="p-6 sm:p-8">
                        <div className="flex items-center gap-3 mb-4">
                            <span className="inline-flex items-center px-3 py-1 rounded-full bg-marigold text-ink text-xs font-bold">
                                New Release
                            </span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 mb-6">
                            {[
                                ...
                                <div key={value} className="rounded-xl p-3 text-center" style={{ background: "var(--surface-warm)" }}>
                                    <p className="text-xs text-ink-soft">{character}</p>
                                    <p className="text-sm font-bold text-evergreen">{value}</p>
                                </div>
                            ))}
                        </div>
                        <Link
                            href="/books/nititales-stories-shape-values"
                            className="inline-flex items-center gap-2 bg-evergreen text-white font-extrabold px-6 py-3 rounded-full hover:bg-evergreen-deep/90 transition-colors"
                        >
                            Explore Nititales →
                        </Link>
                    </div>
